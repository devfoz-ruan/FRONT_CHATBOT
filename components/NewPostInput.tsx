...
import { PostPayload, Posts } from "@/app/models/Posts";
...
export default function NewPostInput() {
  ...
  const trimmedText = TextPost.trim();

  if (trimmedText.length === 0) return;

  const PostPayload: PostPayload = {
    idgrupo: Number(idgrupo),
    idvendedor: Number(localStorage.getItem("dataUser")),
    texto: trimmedText,
    timestamp: new Date(), // ✅ explícito
    iscomentario: false
  };
  ...
  try {
    ...
    if (!response.ok) {
      console.error("Erro ao enviar post:", response.statusText);
      return;
    }
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center justify-between mt-4">
            ...
            <button
              ...
              className="bg-green-600 text-white px-6 py-2.5 rounded-xl font-medium hover:bg-green-700 transition-all shadow-lg shadow-green-200 hover:shadow-green-300 flex items-center gap-2"
            >
              <span>Publicar</span>
              <Send size={18} />
            </button>
